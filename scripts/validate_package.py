...
def load_layout_manifest(
    ...
    def validate_mapping(source: str, destination: str) -> None:
        ...
        if is_maintainer_only_archive_entry(
            source
        ) or is_maintainer_only_archive_entry(destination):
            raise ValueError(
        ...
        if (
            ...
            or any(
                part in LAYOUT_EXCLUDED_PARTS for part in destination_path.parts
            )
            or source_path.suffix in LAYOUT_EXCLUDED_SUFFIXES
            or destination_path.suffix in LAYOUT_EXCLUDED_SUFFIXES
        ):
            raise ValueError(f"excluded layout mapping: {source} -> {destination}")
    ...
    *,
    ...
    if canonical.is_file():
        ...
        if obligation_workflow_error is not None:
            fail(errors, obligation_workflow_error)
