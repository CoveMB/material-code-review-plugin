...
Each skill version receives one fresh primary-reviewer trial in its own detached target clone. The evaluator captures each complete finding ledger and native Gate-A result. For missed-contracts only, each primary reviewer first pauses after coverage recording; a separate zero-history challenger receives frozen source plus coverage/assignment summaries and limitations, but candidate findings, expected roots, variant identities, refs, and the other variant are forbidden. It returns only `NO_COVERAGE_GAP` or a bounded `COVERAGE_GAP` and cannot add findings or count as independent finding validation.
