...
New codebase-scope and delegated change-scope simplification runs deliberately retain `material-review/state/v1` and `material-review/candidate-set/v1` with the exact trusted `profile=material-code-simplification`. They do not require a material-review coverage plan. An unprofiled or contradictory `state/v1` run cannot inherit simplification authority from the adapter and must restart. The standalone adapter remains version 1.2.0.
