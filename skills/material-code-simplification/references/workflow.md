...
Both codebase and delegated change scopes use `material-review/state/v1` and candidate-set/v1 only when state carries the exact `profile=material-code-simplification`. The adapter does not grant authority to an unprofiled or contradictory v1 run; it must restart. Adapter 1.3.0 embeds shared controller and obligation helper 1.4.0 for runtime provenance, but this profile-specific v1 contract does not select change units, coverage-plan/v2, candidate-set/v3, assignments, check results, or review obligations from current material-review state/v3.
