...
| context with verified coverage | `ingest-candidates --input ...` | accepts exactly one complete assignment-matched candidate-set/v5 wave and writes candidates-normalized/v5; findings may be empty but required checks, obligation evidence items, specialist evidence, and paths may not |
