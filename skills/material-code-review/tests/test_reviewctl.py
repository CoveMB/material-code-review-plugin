...
class ReviewCtlTest(unittest.TestCase):
    ...
    def make_run_state_with_frozen_fixture(
        self, *, controller_path: Path, expected_sha256: str
    ) -> None:
        self.assertEqual(
    ...
    def assert_restart_only_state_with_frozen_fixture(
        self,
        *,
        state_version: str,
        ...
        self.make_run_state_with_frozen_fixture(
            controller_path=controller_path,
            expected_sha256=expected_sha256,
        )
        state = self.load("state.json")
        self.assertEqual(
            state["schema_version"], f"material-review/state/{state_version}"
        )
        self.assertEqual(
            reviewctl.classify_state_contract(state),
            f"legacy_material_review_{state_version}",
        )
        ...
        coverage = self.write_json(
            f"state-{state_version}-coverage.json", self.coverage_plan(scope_hash)
        )
    ...
    def test_state_v2_review_is_legacy_after_state_v3_release(self) -> None:
        scope_hash = self.init()
        self.make_run_state_with_frozen_fixture(
            controller_path=CONTROLLER_1_3_COMPAT,
            expected_sha256=CONTROLLER_1_3_COMPAT_SHA256,
        )
        state = self.load("state.json")
    ...
    def test_state_v5_review_is_restart_only_after_state_v6_release(self) -> None:
        scope_hash = self.init()
        self.make_run_state_with_frozen_fixture(
            controller_path=CONTROLLER_1_6_COMPAT,
            expected_sha256=CONTROLLER_1_6_COMPAT_SHA256,
        )
        state = self.load("state.json")
    ...
    def test_state_v4_review_is_restart_only_after_state_v5_release(self) -> None:
        self.assert_restart_only_state_with_frozen_fixture(
            state_version="v4",
            controller_path=CONTROLLER_1_5_COMPAT,
    ...
    def test_state_v3_review_is_restart_only_after_state_v4_release(self) -> None:
        self.assert_restart_only_state_with_frozen_fixture(
            state_version="v3",
            controller_path=CONTROLLER_1_4_COMPAT,
