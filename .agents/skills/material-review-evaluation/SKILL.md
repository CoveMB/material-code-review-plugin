...
7. For Discogs, select the clone source once. Prefer an explicitly supplied or already known trusted local clone source only when its normalized `origin` matches the case repository, it contains both exact case commits, and its porcelain status is empty. Capture that checkout's path, `HEAD`, status, and remotes. Do not search unrelated directories. If no eligible local checkout is known, select the case's public HTTPS repository and record that no active Discogs checkout was used. For missed-contracts, do not use an active target checkout: copy the allowlisted base tree into a run-owned temporary directory and follow the fixture object-format contract below. Require initialization to succeed, immediately require the attestation output to equal exactly `sha1`, and stop before staging, committing, or worker dispatch if either command fails or the value differs. Only after that attestation may you commit the base with the exact case identity/message/timestamp and `commit.gpgsign=false`, overlay only the allowlisted review files, and commit again with signing disabled. Verify both tree hashes, both exact commits, immediate parentage, changed paths, and clean status before any worker dispatch. Never fall back to a host-default object format or a different fixture identity.
...
required_format=sha1
attestation_timing=before-add-commit-or-dispatch
failure=hard-stop-no-fallback
...
For `case:missed-contracts` only, require each primary reviewer to pause after the native controller has frozen context and recorded its current coverage plan plus the complete core, obligation, supplemental, and specialist assignment set, but before candidate ingestion and Gate A. Capture a read-only declarative summary of frozen context identity, change-unit owners/consumers, controlled-risk decisions, specialist scenario decisions, obligations, exact assignment paths/checks, machine-owned obligation check contracts, and limitations. Do not include candidate findings, candidate sets, assignment `check_results`, adjudication, the ledger, expected roots, variant identity, refs, private mapping, prior output, or the other variant.

Immediately dispatch one zero-history challenger per anonymous variant with `fork_turns=none`, its distinct read-only fixture clone, that variant's declarative coverage summary, and `evaluations/material-code-review/prompts/challenger.md`. The challenger is case-only: it returns `NO_COVERAGE_GAP` or a bounded `COVERAGE_GAP` over missing or duplicated change units, unsupported risk decisions, missing or mismatched obligations or obligation check contracts, compound or mismatched assignments, or limitations that defeat the declaration. `NO_COVERAGE_GAP` certifies only that bounded declarative claim; it never establishes that any check result or evidence item was performed, fresh, complete, unblocked, resolved, or safe. The challenger cannot add findings, repair artifacts, act as an independent finding validator, progress a gate, or mutate source. Persist its response as `variant-a/challenge.md` or `variant-b/challenge.md`, then allow the corresponding primary reviewer to continue without revealing the challenge response as candidate-generation guidance. Native controller validation and evaluator-root acceptance of assignments, obligations, check contracts, `check_results`, and Gate-A evidence remain mandatory and fail closed independently of the challenger result.
...
- coverage-plan, assignment path/check authority, obligation check contracts and evidence items, specialist scenarios, check results, and Gate-A evidence are controller-valid independently of the challenger;
