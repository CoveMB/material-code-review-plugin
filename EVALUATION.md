...
For missed-contracts only, one zero-history coverage challenger runs per variant after coverage recording and before Gate A. It sees frozen source, coverage-plan and assignment summaries, and limitations. Candidate findings, expected roots, variant identities, refs, and the other variant are forbidden. It reports only `NO_COVERAGE_GAP` or a bounded gap and cannot add findings, repair artifacts, or independent validation weight.
...
A fresh read-only judge checks anonymous artifacts against the frozen source and returns exactly `VARIANT_A_STRONGER`, `VARIANT_B_STRONGER`, `MATERIAL_TIE`, or `INSUFFICIENT_EVIDENCE`. Invalid or missing required reviewer evidence is preserved as a limitation, never reconstructed, and yields `INSUFFICIENT_EVIDENCE`. For missed-contracts, a challenger gap, incomplete obligation evidence, missing required root, lost baseline material finding, unsupported high-severity addition, invalid Gate-A evidence, or mutation blocks a successful-strengthening claim. Identities are revealed only after the judgment is written. The committed evaluator skill, prompts, case contracts, and rubric own the detailed workflow and comparison semantics; this document only describes the boundary.
