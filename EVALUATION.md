...
Run the evaluation only from a fresh root-controlled material-review task against `CoveMB/material-code-review-plugin#3`, exact host base `8ebeb7ae2a1f28acfe297c258f703865280c4fa4`, and exact host head `c740131b0953a04a93cbe1c970dcbf36dae8bca1`. Initialize `scope:pull_request` with matching provenance and refs so the controller freezes the effective merge-base-to-head diff, use `depth:full` and `external-review:off`, require the complete lens roster, and stop at Gate A without edits. A PR lookup or host-pair verification failure is terminal; never substitute the head parent or `scope:range`.
